        ...
        if(layeredView)
        {
            for(int layer = layeredView->layersCount() - 1; layer >= 0; --layer)
            {
                ...
                if(d->userLayerPoolable)
                {
                    ...
                    foreach(medLayerParameterGroupL *layerGroup, layerGroups)
                    {
...
void medAbstractWorkspaceLegacy::removeLayer()
{
    ...
    if(layerView)
    {
        ...
        {
            if ( medViewContainerManager::instance()->container(containerUuid)->closingMode() == medViewContainer::CLOSE_CONTAINER )
            {
...
{
    foreach(medViewParameterGroupL* group, medParameterGroupManagerL::instance()->viewGroups(this->identifier()))
        addViewGroup(group);
}
